...
//    input [3:0] VAUXP, VAUXN,  // Auxiliary analog channel inputs
    ...
    input  i2c_sclk_in,     // Buffer output
    output i2c_sclk_ts,      // 3-state enable input, high=input, low=output
    input  i2c_sda_in,     // Buffer output
    output i2c_sda_ts,      // 3-state enable input, high=input, low=output
...
SYSMONE4 #(// Initializing the SYSMON Control Registers
    ...
    .INIT_42(16'h0800),// Set DCLK divides Maq adjusted for 100M, was 0400 for 50M
...
SYSMON_INST (// Connect up instance IO. See UG580 for port descriptions
    ...
    .I2C_SCLK   (i2c_sclk_in), // not used
    .I2C_SCLK_TS   (i2c_sclk_ts), // not used
    .I2C_SDA    (i2c_sda_in), // not used
    .I2C_SDA_TS    (i2c_sda_ts), // not used
    .OT     (OT),
    .MUXADDR    (),// not used
    .VP     (VP),
    .VN     (VN)
...
//    assign vauxn_active = {12'h000, VAUXN[3:0]};
    assign vauxp_active = 16'h0000;
    assign vauxn_active = 16'h0000;
