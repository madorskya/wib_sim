...
# updated 25 April 2023 for PTCv4 test
...
function cdr_reset {
    devmem 0xa00c0004 32 0x0001000a
    i2cset -y 0 0x40 9 0x68
    i2cset -y 0 0x40 9 0
    i2cget -y 0 0x40 4
    devmem 0xa00c0004 32 0x00010000
}
...
function print_ept_status {
        ...
        # bottom four bits are the endpoint status, bits 17:16 are the cdr_los a
nd cdr_lol respectively
        ept_reg_val=$(peek ${ept_reg})
        printf "EPT CDR LOS: 0x%x\n" $(( (ept_reg_val >> 18) & 0x1 )) # bit 18 i
s CDR LOS as seen by endpoint
        printf "EPT CDR LOL: 0x%x\n" $(( (ept_reg_val >> 17) & 0x1 )) # bit 17 i
s CDR LOL as seen by endpoint
        printf "EPT TS RDY:  0x%x\n" $(( (ept_reg_val >> 5 ) & 0x1 )) # bit 5 is
 ts ready
        printf "EPT STATE:   0x%x\n" $((  ept_reg_val & 0x0f )) # bits 3:0 are t
he endpoint state
...
#uf/clock/si5345_config
soft_debug_p3/clock/a.out
